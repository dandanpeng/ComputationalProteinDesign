...
# -*- coding: utf-8 -*-
"""
Created on Fri Jul 19 22:33:00 2019
...
def genetic_algorithm(protein, match, popSize, eliteSize, num_points, mutationRate, generations):
    ...
    def crossover_population(matingpool, num_points):
        children = np.empty(shape = (matingpool.shape[0], matingpool.shape[1]), dtype = int)
        for i in range(len(matingpool)):
            parent1_idx = i % matingpool.shape[0]
            parent2_idx = (i+1) % matingpool.shape[0]
            children[i] = crossover(matingpool[parent1_idx], matingpool[parent2_idx], num_points)
        return children
    ...
    def next_generation(population, eliteSize, num_points, mutationRate):
        matingpool, best = selection(population, eliteSize)
        children = crossover_population(matingpool, num_points)
        next_generation = mutate_population(children, mutationRate)
        
        fitnessResults = {}
        for i in range(len(children)):
            fitnessResults[i] = energy(next_generation[i])
        sortedResults = sorted(fitnessResults.items(), key = operator.itemgetter(1), reverse = True)
        
        lowest = sortedResults[-1]
        if best[1] > lowest[1]:            
            next_generation[lowest[0]] = matingpool[best[0]]
